...
const sagas = function* saga() {
  yield all([
    // SEARCH_POKEMON
    fork(function* () {
      yield takeLeading(ActionTypes.SEARCH_POKEMON, function* (action) {
        const {
          search = "",
        } = action.payload;

        let offset = 0;
        if (previousSearchTerm === search) {
    ...
    // GET_BAG
    fork(function* () {
      yield takeLeading(ActionTypes.GET_BAG, function* (action) {
        try {
    ...
    // ADD_TO_BAG
    fork(function* () {
      yield takeLeading(ActionTypes.ADD_TO_BAG, function* (action) {
        try {
    ...
    // REMOVE_FROM_BAG
    fork(function* () {
      yield takeLeading(ActionTypes.REMOVE_FROM_BAG, function* (action) {
        try {
